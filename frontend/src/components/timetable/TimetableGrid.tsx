      ...
      <div className="flex w-max min-w-full flex-col">
        <div className="flex border-b bg-muted/50">
          ...
          {DAYS.map((day) => (
            <div
              key={day}
              className="flex-1 min-w-[200px] border-r p-4 font-semibold text-center last:border-r-0"
            >
              {day}
            </div>
          ))}
          ...
          <div className="flex border-b last:border-b-0 min-h-[110px]" key={time}>
            ...
            {DAYS.map((day) => {
              ...
              return (
                <div
                  key={`${day}-${time}`}
                  className="flex-1 min-w-[200px] border-r p-2 last:border-r-0"
                >
                  {period && period.subject && period.teacher ? (
                    <div className="h-full w-full rounded-md border bg-card p-3 shadow-sm hover:shadow-md transition-all flex flex-col justify-between gap-2 border-l-4 border-l-primary">
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground mt-auto pt-2 border-t border-dashed">
                        ...
                        <span className="truncate max-w-[140px]" title={period.teacher.name}>
                          {period.teacher.name}
                        </span>
                      </div>
                    </div>
