...
const ClassForm = ({ open, onOpenChange, initialData, onSuccess }: Props) => {
  ...
  const yearOptions = years.map((year) => ({ label: year.name, value: year._id }));
  const subjectOptions = subjects.map((subject) => ({ label: subject.name, value: subject._id }));
  const teachersOptions = teachers.map((teacher) => ({ label: teacher.name, value: teacher._id }));
