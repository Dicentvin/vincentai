...
interface ChatBody { documentId?: string; query?: string; }
...
export default async function handler(req: VercelReq, res: VercelRes): Promise<void> {
  ...
  const { documentId, query } = await parseBody<ChatBody>(req);
  ...
  try {
    ...
    if (documentId && documentId !== "dashboard") {
      ...
      if (doc) {
        ...
        contextText = chunks.length
          ? chunks.slice(0, 6).map(c => c.content).join("\n\n")
    ...
    if (documentId && documentId !== "dashboard") {
      const { data: existing } = await supabase
        .from("chat_history").select("id, messages")
        .eq("user_id", user.id).eq("document_id", documentId).maybeSingle();

      const messages: { role: string; content: string }[] = existing?.messages || [];
      messages.push({ role: "user", content: query });
      messages.push({ role: "assistant", content: answer });

      if (existing) {
        await supabase.from("chat_history")
          .update({ messages, updated_at: new Date().toISOString() }).eq("id", existing.id);
      } else {
        await supabase.from("chat_history")
